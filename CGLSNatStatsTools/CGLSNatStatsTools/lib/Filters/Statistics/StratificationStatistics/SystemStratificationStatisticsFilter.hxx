...
#ifndef SYSTEMSTRATIFICATIONSTATISTICSFILTER_HXX
#define SYSTEMSTRATIFICATIONSTATISTICSFILTER_HXX
...
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::Reset() {
    currentRegionId = 1;
}
...
template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::Synthetize() {
...
template <class TInputImage, class TPolygonDataType>
bool SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::ValidAOI() {
    return !(aoi.MaxX == 0 && aoi.MinX == 0 && aoi.MinY == 0 && aoi.MaxY == 0);
}

template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::GenerateInputRequestedRegion() {
...
template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::GenerateOutputInformation() {
...
template <class TInputImage, class TPolygonDataType>
SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::SystemStratificationStatisticsFilter():currentRegionId(1) {
    this->SetNumberOfRequiredInputs(1);
}

template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::AfterThreadedGenerateData() {
    currentRegionId++;
}

template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::BeforeThreadedGenerateData() {
    std::cout <<"Processing Region with id: " << currentRegionId <<"\n";
}

template <class TInputImage, class TPolygonDataType>
typename TInputImage::Pointer SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::GetReferenceImage() {
    return static_cast<TInputImage*>(this->ProcessObject::GetInput(0));
}

template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::ThreadedGenerateData(const RegionType& outputRegionForThread, itk::ThreadIdType threadId) {
...
template <class TInputImage, class TPolygonDataType>
typename SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::RegionData SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::rasterizer(typename TInputImage::RegionType region, itk::ThreadIdType threadId) {
    ...
    std::string query = polygonInfoQuery(evlp);
    PGPool::PGConn::UniquePtr cn  = PGPool::PGConn::New(Configuration::connectionIds[config->statsInfo.connectionId]);
...
template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::prepareImageInfo(JsonValue &images) {
...
template <class TInputImage, class TPolygonDataType>
void SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::processGeomIdsAndImages(JsonDocumentSharedPtr polyIds, JsonValue &images) {
...
template <class TInputImage, class TPolygonDataType>
std::string SystemStratificationStatisticsFilter<TInputImage, TPolygonDataType>::polygonInfoQuery(OGREnvelope &envelope) {
    return fmt::format("with region AS( select st_setsrid((st_makeenvelope({0},{1},{2},{3})),4326) bbox) select sg.id, ST_ASTEXT(ST_MULTI(case when ST_CONTAINS(region.bbox, sg.geom) THEN sg.geom ELSE st_intersection(sg.geom, region.bbox) END)) geom FROM stratification_geom sg JOIN region ON TRUE WHERE sg.id IN ({4}) AND st_intersects(sg.geom, region.bbox)",
                       envelope.MinX, envelope.MinY,envelope.MaxX, envelope.MaxY, polyIdsStr);
}
}
#endif // SYSTEMSTRATIFICATIONSTATISTICSFILTER_HXX
