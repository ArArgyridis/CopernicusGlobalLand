...
class DBImporter:
    ...
    def __migrateData(self):
        ...
        try:
            ...
            """ INSERT INTO {0}.stratification_geom (stratification_id, geom, geom3857, description) SELECT s.id, ST_MakeValid(ST_Transform({2},4326)),
             ST_MakeValid(ST_Transform({2}, 3857)), dt.{3} FROM {4}.{5} dt JOIN {0}.stratification s
